...
export class EmployeeService {
  private baseUrl = 'https://http-1-71708-default-rtdb.firebaseio.com/employeesList';
  ...
  getAllEmployees(): Observable<IEmployee[]> {
    return this.http.get<{ [key: string]: IEmployee }>(this.getEmployeeUrl()).pipe(
      map((response) => {
  ...
  addEmployee(employee: IEmployee): Observable<IEmployee> {
    return this.http.post<IEmployee>(this.getEmployeeUrl(), employee).pipe(
      catchError(this.handleError)
    );
  }

  editEmployee(employee: IEmployee): Observable<IEmployee> {
    return this.http.put<IEmployee>(this.getEmployeeUrl(employee.id), employee).pipe(
      catchError(this.handleError)
    );
  }

  deleteEmployee(id: string): Observable<void> {
    return this.http.delete<void>(this.getEmployeeUrl(id)).pipe(
      catchError(this.handleError)
    );
  }
